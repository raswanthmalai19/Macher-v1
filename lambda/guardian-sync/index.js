...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (path.includes('/alerts')) {
      if (method === 'POST') return await handlePushAlert(event);
      if (method === 'GET') return await handleGetAlerts(event);
    }
...
async function handlePushAlert(event) {
  const body = JSON.parse(event.body || '{}');
  const { alertId, guardianId, protectedUserId, threatLevel, threatType, confidence, actionTaken, callerNumber, timestamp } = body;

  if (!alertId || !guardianId) {
    return response(400, { error: 'Missing alertId or guardianId' });
  }

  // Store alert in a separate sort key space under the guardian's partition
  await ddb.send(new PutCommand({
    TableName: TABLE_NAME,
    Item: {
      linkId: `alert#${alertId}`,
      guardianId,
  ...
  return response(201, { alertId, status: 'stored' });
}
...
async function handleGetAlerts(event) {
  ...
  const result = await ddb.send(new QueryCommand({
    ...
    IndexName: 'guardianId-index',
    KeyConditionExpression: 'guardianId = :gid',
    FilterExpression: 'begins_with(linkId, :prefix)',
    ExpressionAttributeValues: {
      ':gid': guardianId,
      ':prefix': 'alert#',
    },
  ...
  const alerts = (result.Items || []).map(item => ({
    alertId: item.linkId.replace('alert#', ''),
    guardianId: item.guardianId,
    protectedUserId: item.protectedUserId,
