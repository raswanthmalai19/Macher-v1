 ...
 */
async function getOrCreateCallSession(callSessionId: string, connectionId: string): Promise<CallSessionState> {
  // Check in-memory cache first (warm Lambda reuse)
  let session = callSessions.get(callSessionId);
  if (session) {
    return session;
  }

  // Try to load from DynamoDB (persisted across Lambda invocations)
  try {
    const result = await docClient.send(
      new GetCommand({
        ...
        Key: {
          callSessionId,
          timestamp: 0, // Reserved key for session state
        },
      })
    );

    if (result.Item && result.Item.recordType === 'SESSION_STATE') {
      session = {
        callSessionId: result.Item.callSessionId,
        connectionId,
        startTime: result.Item.startTime,
        transcriptionBuffer: result.Item.transcriptionBuffer || [],
        wordCount: result.Item.wordCount || 0,
        speechDuration: result.Item.speechDuration || 0,
        lastAnalysisTime: result.Item.lastAnalysisTime || 0,
        analysisCount: result.Item.analysisCount || 0,
        currentRiskScore: result.Item.currentRiskScore || 0,
        currentThreatLevel: result.Item.currentThreatLevel || 'SAFE',
      };
      callSessions.set(callSessionId, session);

      log({
        level: 'INFO',
        message: 'Call session restored from DynamoDB',
        callSessionId,
        connectionId,
    ...
    log({
      level: 'WARN',
      message: 'Failed to load session from DynamoDB, creating new',
      callSessionId,
      error: {
...
/**
 * Persist call session state to DynamoDB so it survives across Lambda invocations.
 */
async function saveSessionToDynamoDB(session: CallSessionState): Promise<void> {
  try {
    ...
    log({
      level: 'ERROR',
      message: 'Failed to persist session to DynamoDB',
      callSessionId: session.callSessionId,
      error: {
        name: (error as Error).name,
        message: (error as Error).message,
  ...
  try {
    ...
    const fraudThresholdResponse = await ssmClient.send(
      new GetParameterCommand({
        Name: `/macher/${ENVIRONMENT}/audio-processor/fraud-threshold`,
      })
    );
    ...
    const maxProcessingTimeResponse = await ssmClient.send(
      new GetParameterCommand({
        Name: `/macher/${ENVIRONMENT}/audio-processor/max-processing-time`,
      })
    );
