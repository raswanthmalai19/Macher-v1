...
export interface FraudDetectionConstructProps {
  ...
  notificationServiceUrl?: string; // Optional notification service URL
}
...
export class FraudDetectionConstruct extends Construct {
  ...
  constructor(scope: Construct, id: string, props: FraudDetectionConstructProps) {
    super(scope, id);
    ...
    analyzeResource.addMethod(
      'POST',
      new apigateway.LambdaIntegration(this.analysisFunction, {
        proxy: true,
