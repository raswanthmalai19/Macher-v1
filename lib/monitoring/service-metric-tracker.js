...
exports.serviceMetricTracker = exports.ServiceMetricTracker = void 0;
...
class ServiceMetricTracker {
     ...
     */
    async trackTranscribeProcessing(audioDurationMs, processingTimeMs) {
        try {
            ...
            if (audioDurationMs <= 0 || processingTimeMs < 0) {
                structured_logger_1.logger.warn('Invalid Transcribe metrics', {
            ...
            await this.metricPublisher.publishMetric('VocalShield/Services', 'TranscribeProcessingTime', processingTimeMs, types_1.MetricUnit.Milliseconds, dimensions, timestamp);
            // Publish audio duration
            await this.metricPublisher.publishMetric('VocalShield/Services', 'TranscribeAudioDuration', audioDurationMs, types_1.MetricUnit.Milliseconds, dimensions, timestamp);
            ...
            await this.metricPublisher.publishMetric('VocalShield/Services', 'TranscribeEfficiencyRatio', processingTimePerSecond, types_1.MetricUnit.Milliseconds, dimensions, timestamp);
            structured_logger_1.logger.info('Transcribe metrics tracked', {
                component: 'ServiceMetricTracker',
            ...
            structured_logger_1.logger.error('Failed to track Transcribe metrics', error, {
                component: 'ServiceMetricTracker',
                metadata: {
                    audioDurationMs,
                    processingTimeMs,
     ...
     */
    async trackBedrockInvocation(responseTimeMs, inputTokens, outputTokens) {
        try {
            const timestamp = new Date();
            const dimensions = {
                Service: 'Bedrock',
            ...
            if (responseTimeMs < 0 || inputTokens < 0 || outputTokens < 0) {
                structured_logger_1.logger.warn('Invalid Bedrock metrics', {
                    component: 'ServiceMetricTracker',
                    metadata: {
                        responseTimeMs,
                        inputTokens,
            ...
            await this.metricPublisher.publishMetric('VocalShield/Services', 'BedrockResponseTime', responseTimeMs, types_1.MetricUnit.Milliseconds, dimensions, timestamp);
            // Publish input tokens
            await this.metricPublisher.publishMetric('VocalShield/Services', 'BedrockInputTokens', inputTokens, types_1.MetricUnit.Count, dimensions, timestamp);
            // Publish output tokens
            await this.metricPublisher.publishMetric('VocalShield/Services', 'BedrockOutputTokens', outputTokens, types_1.MetricUnit.Count, dimensions, timestamp);
            // Publish total tokens
            await this.metricPublisher.publishMetric('VocalShield/Services', 'BedrockTotalTokens', totalTokens, types_1.MetricUnit.Count, dimensions, timestamp);
            structured_logger_1.logger.info('Bedrock metrics tracked', {
                component: 'ServiceMetricTracker',
            ...
            structured_logger_1.logger.error('Failed to track Bedrock metrics', error, {
                component: 'ServiceMetricTracker',
                metadata: {
                    responseTimeMs,
                    inputTokens,
     ...
     */
    getTranscribeMetrics(audioDurationMs, processingTimeMs) {
        return {
            audioDurationMs,
            processingTimeMs,
            timestamp: new Date(),
     ...
     * @returns Metrics object
     */
    getBedrockMetrics(responseTimeMs, inputTokens, outputTokens) {
        return {
            responseTimeMs,
            inputTokens,
            outputTokens,
...
exports.ServiceMetricTracker = ServiceMetricTracker;
/**
 * Create a singleton service metric tracker instance for convenience
 */
exports.serviceMetricTracker = new ServiceMetricTracker();
//# sourceMappingURL=service-metric-tracker.js.map